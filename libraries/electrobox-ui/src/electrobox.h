...
enum language_item {
	...
	ARE_YOU_SURE_RESET_STATISTICS = 98,
	UPLOAD_FIRMWARE = 99,
	UPDATE_STARTED = 100,
	UPDATE_STOPPED = 101,
...
struct statistics {
	...
	{
		::statistics.dialog.pack(attr::display = false).send(sender);
		packet buf;
		time_t t = now();
		for (int i = 0; i < 5; i++) {
	...
	{
		::statistics.dialog.pack(attr::display = true).send(sender);
	}

	statistics() : yes(yes_callback), no(no_callback), button(button_callback) {};
	packet build()
	{
		...
		buf += button.pack(box, (attr::text = l_str(STATS_RESET))) +
		dialog.pack(root, (attr::display = false)) + wrapper.pack(dialog, (attr::text = l_str(ARE_YOU_SURE_RESET_STATISTICS), attr::direction = DIR_H)) +
...
struct current_regulator {
	...
	packet build()
	{
		return
			...
			range.pack(box, (attr::min = min_value, attr::max = max_value, attr::value = settings.current_regulator));
	}
} current_regulator;
...
struct sta_dialog {
	...
	static void switcher_callback(struct switcher &id, client_id_t sender)
	{
		(id.turn(::sta_dialog.dhcp) + ::sta_dialog.dhcp_text.pack(attr::text = l_str(AUTO_IP)) +
		::sta_dialog.manual_wrapper.pack(attr::display = !::sta_dialog.dhcp)).send_all();
	}
	...
	packet update(int idx)
	{
			...
			status.pack(attr::text = 
				...
				l_str(network::sta.dhcp ? AUTO_IP : MANUAL_IP) + network::get_connected_station_ip().toString() + "\n" +
	...
	packet build()
	{
		...
		dhcp_wrapper.pack(box, (attr::direction = DIR_H)) + dhcp_text.pack(dhcp_wrapper, (attr::text = l_str(AUTO_IP))) + dhcp_enabler.pack(dhcp_wrapper, dhcp_enabler.get(dhcp)) +
...
struct sta_settings {
	...
	struct avail_stations : radio {
		static void select_station_callback(struct radio &id, client_id_t sender)
		{
			...
			sta_dialog.update(idx).send_all();
			sta_dialog.dialog.pack(attr::display = true).send(sender);
		}
...
struct firmware_update_widget {
	...
	{		
		return
			box.pack(navigation_panel.settings, (attr::text = l_str(UPLOAD_FIRMWARE))) + 
			file.pack(box) +
...
bool ui::interface(client &cl, int idx) //implementation of interface builder is user specified
{
	BEGIN_ADD_TO_INTERFACE {
		...
		ADD_TO_INTERFACE(19, firmware_update_widget);
